...
const AdminManageStylists = () => {
  ...
  // Add or update stylist
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      if (editMode) {
        // UPDATE
        await axios.put(`http://localhost:5000/api/stylists/${currentId}`, form);
      ...
      alert(err.response?.data?.message || 'Error saving stylist');
    }
  };
  ...
  <div className="ssidebar">
      ...
      <div className="stylist-main">
         <h2>{editMode ? 'Edit Stylist' : 'Add New Stylist'}</h2>
        <form onSubmit={handleSubmit} className="stylist-form">
          <input name="name" placeholder="Name" value={form.name} onChange={handleChange} required />
          <input name="email" placeholder="Email" value={form.email} onChange={handleChange} required />
          <input name="password"    placeholder={editMode ? 'Leave blank to keep current password' : 'Password'} type="password" value={form.password} onChange={handleChange} required     {...(!editMode && { required: true })}/>
          ...
          <button type="submit">{editMode ? 'Save Changes' : 'Add Stylist'}</button>
